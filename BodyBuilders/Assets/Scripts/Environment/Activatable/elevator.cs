...
public class elevator : activate
{
    ...
    bool launchedPlayer = false; // has the elevator applied a force to the player at the peak of it's slam
    ...
    void FixedUpdate()
    {
        distanceMoved = new Vector2(transform.position.x , transform.position.y) - previousPosition;
        ...
        {
            if(ascending) launchedPlayer = false;
            ascending = !ascending;
            moveTimer = 0f;
        }
        ...
        if(overcharge && ascending)
        {
            if(moveTimer >= (1f - jumpTimeOffset) && overcharge) // when nearing the apex, the elevator gives the player a brief period to time to jump and use the boost
            {
                ...
                if(!playerOnboard)
                {
                    Debug.Log("Player is not on board");
                }
            }
...
/*
        ...
        {
            child.transform.position = distanceMoved + (Vector2)child.transform.position;
        }
*/
    ...
    void OnCollisionEnter2D(Collision2D col)
    {
        ...
        {
            Debug.Log("OnIt");
            NewParent(col.gameObject);
            Parent(col.gameObject);
            //col.otherRigidbody.AddForce(30f * Vector2.up , ForceMode2D.Impulse);
        }
    }
...
*/
    ...
    void NewParent(GameObject child)
    {
    ...
    void Unparent(GameObject child)
    {
        ...
        {
            if(child.gameObject.transform.parent != null && child.gameObject.transform.parent == holder)
            {
                child.gameObject.transform.parent = null;
            }
            else if(child.gameObject.transform.parent.parent != null && child.gameObject.transform.parent.parent == holder)
            {
                child.gameObject.transform.parent.gameObject.transform.parent = null;
            }
        }
    }
    ...
    void Parent(GameObject child)
    {
        ...
        if(Input.GetAxisRaw("Vertical") <= 0.1f && (child.tag != "Untagged" || child.tag != "PassThroughPlatform" || child.tag != "Environment"))
        {
            ...
            else if(child.tag == "Arms" || child.tag == "Legs") // reference the correct transform for legs and arms (they use multiple childed game objects)
            {
                child.gameObject.transform.parent.parent = holder.transform;
            }
            else if(child.tag == "Box" || child.tag == "Powercell") // child Boxes and Powercells normally
            {
                child.gameObject.transform.parent = holder.transform;
            }
            else // generic check for parent, this will find whatever object is highest in the parent hierarchy, and make it the child of the holder
            {
                if(child.gameObject.transform.parent == null)
                {
                    child.gameObject.transform.SetParent(holder.transform);
                }
                else if(child.gameObject.transform.parent.parent == null)
                {
                    child.gameObject.transform.parent.SetParent(holder.transform);
                }
                else if(child.gameObject.transform.parent.parent.parent == null)
                {
                    child.gameObject.transform.parent.parent.SetParent(holder.transform);
                }
                else if(child.gameObject.transform.parent.parent.parent.parent == null)
                {
                    child.gameObject.transform.parent.parent.parent.SetParent(holder.transform);
                }
            }
        }
    }
    ...
    // issue is the player is not in the collider, so they are likely not 
        ...
        void OnTriggerStay2D(Collider2D col)
    {
        ...
        {
            if(col.tag == "Player" && !launchedPlayer)
            {
                ...
                if(Input.GetAxisRaw("Vertical") > 0f) // if the player is holding up, apply extra force, otherwise apply normal force
                {
                    ...
                    //Debug.Log("jump force = " + slamDirection * jumpLaunchForce);
                }
                else
                {
                    player.GetComponent<Rigidbody2D>().AddForce(slamDirection * launchForce , ForceMode2D.Impulse);
                    //Debug.Log("normal force = " + slamDirection * launchForce);
                }
        ...
        if(ascending && overcharge)
        {
            triggerCheckTime = slamUpTime / 30f;
        }
        else
        {
            triggerCheckTime = moveTime / 30f;
        }
        ...
        if(!overcharge && playerOnboard && Input.GetAxisRaw("Vertical") > 0)
        {
            pickupPoint.enabled = false;
        }
